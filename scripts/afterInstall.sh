...
sudo docker build -t somparty:latest /home/ubuntu/somparty/.

# 3. ECR로 이미지 태그 추가 및 푸시
echo "Tagging and pushing the Docker image to ECR..."
sudo docker tag somparty:latest 084828566517.dkr.ecr.ap-northeast-2.amazonaws.com/somparty:latest
sudo docker push 084828566517.dkr.ecr.ap-northeast-2.amazonaws.com/somparty:latest

# 4. 기존 Docker Compose 서비스 중지 및 제거
